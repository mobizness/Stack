...
app.directive('usageChart', ['$timeout', 'Report', '$routeParams', 'COLOURS', function($timeout, Report, $routeParams, COLOURS) {
  ...
  var link = function(scope,element,attrs,controller) {
    ...
    function chart() {
      ...
      data = {
          usage : {
              inbound: 1 - (scope.outbound / scope.totalbound),
              outbound: (scope.outbound / scope.totalbound)
          }
      }
      data = {
          usage : scope.wifiUsageValues
      }
        if (!Number(data.usage.inbound) && !Number(data.usage.outbound)) {
...
app.directive('loadChart', ['Report', '$routeParams', '$timeout', function(Report, $routeParams, $timeout) {
  ...
  var link = function(scope,element,attrs,controller) {
    ...
    function chart() {
      ...
      console.log("scope.loadAverageValues:", scope.loadAverageValues)
        var json = {
            load: scope.loadAverageValues
        }
    ...
    function drawChart(json) {
      ...
      if(!window.google.visualization.DataTable){
          ...
          for(var i = 0; i < len; i++) {
            ...
            var time = new Date(json.load[i].time *1000);
